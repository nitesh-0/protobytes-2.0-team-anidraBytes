...
Supports Depth Anything V2 (GPU) with fallback to bbox-based estimation.
"""
...
class DepthEstimator:
    ...
    def _run_model(self, frame: np.ndarray) -> np.ndarray:
        """Run the depth model on a frame."""
    ...
    def estimate(self, frame: np.ndarray) -> Optional[np.ndarray]:
        ...
        Returns normalized depth map (higher = farther) or None.
    ...
    def get_depth_at_point(self, depth_map: Optional[np.ndarray],
                           x: int, y: int) -> float:
        """Get relative depth value at a pixel coordinate."""
